...
ENT.ChargeToHealth = 150

if SERVER then

    function ENT:CanConsume(ply)
        return ply:Health() < ply:GetMaxHealth() and math.floor(self:GetCharge() * self.ChargeToHealth) > 0
    end

    function ENT:OnConsume(ply)
        local diff = math.min(ply:GetMaxHealth() - ply:Health(), math.floor(self:GetCharge() * self.ChargeToHealth))
        ply:SetHealth(math.min(ply:GetMaxHealth(), ply:Health() + diff))
        self:SetCharge(self:GetCharge() - diff / self.ChargeToHealth)
    ...
    function ENT:OnPropDestroyed(dmginfo)
        ...
        for i = 1, math.floor(self:GetCharge() * self.ChargeToHealth / 10 * math.Rand(0.5, 1)) do
