...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        self.IgnoreRagdoll = 1
        self.BreakConstraints = 1
        ...
        if self.Yield > 0.13 then
            ...
            for i = 1, 6 do
                for k = -1, 1, 2 do
                    for j = -1, 1, 2 do
                        ...
                        --we need to find the closest distance to the ground, so we know where to spawn our nuke
                        if TraceLength < ShortestTraceLength then
            ...
            --if there are more than 10 long traces, we are probably in the air
            if LongOnes > 10 then
                trace.start = self.SplodePos
                ...
                util.Effect("pt_nuke_effect_air", effectdata)
                ...
                util.Effect("pt_nuke_effect_ground", effectdata)

                if self.EpicBlastWave then
                    util.Effect("pt_nuke_blastwave", effectdata)
                else
                    util.Effect("pt_nuke_blastwave_cheap", effectdata)
                end
            end
        ...
        self.SplodeDist = 100
        self.BaseDamage = 1.5e9 * self.Yield
    ...
    function ENT:Think()
        ...
        for key, found in pairs(ents.FindInSphere(self.SplodePos, self.SplodeDist)) do
            ...
            if EntSpecs.valid then
                ...
                if self:LOS(EntSpecs.ent, entpos) then
                    ...
                    if DamagePerSecond >= 250 then
                        --if we've hit a human
                        if EntSpecs.humanoid then
                            local effectdata = EffectData()
                            effectdata:SetOrigin(entpos)
                        ...
                        elseif EntSpecs.type == "npc_strider" then
                            --if we've hit a strider...
                    ...
                    --if it's not a physics object...
                    if EntSpecs.movetype ~= 6 or not physobj:IsValid() then
                        EntSpecs.ent:SetVelocity(vecang * (100 * Damage)) --push it away
                    elseif EntSpecs.ragdoll then
                        --if it's a ragdoll...
    ...
    local HumanModels = {"[Ss]oldier", "models/zombie", "models/player/", "models/Humans/", "[Pp]olice.mdl", "[Aa]lyx.mdl", "[Bb]arney.mdl", "[Bb]reen.mdl", "[Ee]li.mdl", "[Mm]onk.mdl", "[Kk]leiner.mdl", "[Mm]ossman.mdl", "[Oo]dessa.mdl", "[Gg]man"}
