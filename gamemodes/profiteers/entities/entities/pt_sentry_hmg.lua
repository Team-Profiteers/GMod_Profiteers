...
if SERVER then
    ...
    function ENT:Think()
        ...
        if IsValid(self.Target) then
            ...
            targetang = self:WorldToLocalAngles((tgtpos - (self:GetPos() + Vector(0, 0, 48))):Angle())
        else
            targetang = Angle(0, self:WorldToLocalAngles(self:GetAngles()).y + math.sin(CurTime() / math.pi / 3) * 180, 0)
        end

        self:SetAimAngle(Angle(
            math.ApproachAngle(self:GetAimAngle().p, targetang.p, engine.TickInterval() * 600),
            math.ApproachAngle(self:GetAimAngle().y, targetang.y, engine.TickInterval() * 600), 0))
    ...
    function ENT:ShootTarget()
        ...
        self:EmitSound("weapons/m249/m249-1.wav", 130, 90, 0.85)
